...
func testAliyun(disableEPSV bool) {

	c, _ := Dial("139.129.150.243:21", DialWithTimeout(5*time.Second), DialWithDisabledEPSV(disableEPSV))

	err := c.Login("qxu1780860082", "dzh@1234")

	data := bytes.NewBufferString(testData)
	err = c.Stor("/htdocs/test.html", data)
	if err != nil {
		panic(err)
	}
...
func testConn(t *testing.T, disableEPSV bool) {
	assert := assert.New(t)
	mock, c := openConn(t, "139.129.150.243", DialWithTimeout(5*time.Second), DialWithDisabledEPSV(disableEPSV))
...
func TestConnect(t *testing.T) {
	mock, err := newFtpMock(t, "139.129.150.243:21")
	if err != nil {
		t.Fatal(err)
	}
	defer mock.Close()
...
func TestDeleteDirRecur(t *testing.T) {
	mock, c := openConn(t, "139.129.150.243:21")
...
func TestListCurrentDir(t *testing.T) {
	mock, c := openConnExt(t, "139.129.150.243:21", "no-time", DialWithDisabledMLSD(true))
...
func TestTimeVsftpdFull(t *testing.T) {
	mock, c := openConnExt(t, "103.20.192.213:21", "vsftpd", DialWithWritingMDTM(true))
...
func TestDialWithDialFunc(t *testing.T) {
	...
	_, err := Dial("103.20.192.213:21", DialWithDialFunc(f))
...
func TestDialWithDialer(t *testing.T) {
	...
	mock, err := newFtpMock(t, "103.20.192.213:21")
	assert.NoError(t, err)
